...
def shop(request):
    if request.user.is_authenticated:
        ...
        try:
            ...
            cart_user=user
    if cart_user:
        carts = Cart.objects.filter(user=cart_user, status=0)
        list_cart = []
        if carts:
            cart_product_id = [item.productname for item in carts]
            with connection.cursor() as cursor:
    ...
    return render(request, "shop.html", {
        "item": Shop_Item.objects.all(),  # Changed to "items" to match the variable name
        "cart": list_cart
    })
@csrf_exempt
        ...
        with connection.cursor() as cursor:
            ...
            if result:
                ...
                with connection.cursor() as cc:
                     print(87)
                     cc.execute("INSERT INTO fitness_Cart (user_id, productname, quantity, status) VALUES (%s, %s, %s, %s)", [buyer.id, product_id, 1, 0])
